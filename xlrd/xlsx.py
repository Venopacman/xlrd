...
def ensure_elementtree_imported(verbosity, logfile):
    ...
    Element_has_iter = hasattr(ET, 'ElementTree') and hasattr(
        ET.ElementTree, 'iter')
    if verbosity:
        etree_version = repr([(item, getattr(ET, item))
...
def cell_name_to_rowx_colx(cell_name,
    ...
    try:
            ...
            if lv:
                ...
                if charx == 0:
                    ...
                    if allow_no_col:
                        ...
                        raise Exception('Missing col in cell name %r',
                                        cell_name)
                else:
                    colx = colx - 1
        ...
        raise Exception('Unexpected character %r in cell name %r' %
                        (c, cell_name))
...
def get_text_from_si_or_is(self,
                           elem,
                           r_tag=U_SSML12 + 'r',
                           t_tag=U_SSML12 + 't'):
...
class X12General(object):
    def process_stream(self, stream, heading=None):
        ...
        ) if Element_has_iter else self.tree.getiterator():
            if self.verbosity >= 3:
                self.dump_elem(elem)
    ...
    def dump_elem(self, elem):
        fprintf(self.logfile, "===\ntag=%r len=%d attrib=%r text=%r tail=%r\n",
                split_tag(elem.tag)[1], len(elem), elem.attrib, elem.text,
                elem.tail)
...
class X12Book(X12General):
    ...
    def do_sheet(self, elem):
        ...
        if self.verbosity >= 2:
            self.dumpout('sheetx=%d sheetId=%r rid=%r type=%r name=%r', sheetx,
                         sheetId, rid, reltype, name)
...
class X12Styles(X12General):
    def __init__(self, bk, logfile=DLF, verbosity=0):
        ...
        self.xf_type = None
        self.fmt_is_date = {}
        for x in list(range(14, 23)) + list(range(
                45, 48)):  #### hard-coding FIX ME ####
    ...
    def do_numfmt(self, elem):
        ...
        if self.verbosity >= 3:
            self.dumpout('numFmtId=%d formatCode=%r is_date=%d', numFmtId,
                         formatCode, is_date)
...
class X12Sheet(X12General):
    ...
    def do_merge_cell(self, elem):
        ...
        if ref:
            ...
            self.merged_cells.append((first_rowx, last_rowx + 1, first_colx,
                                      last_colx + 1))
    ...
    def do_row(self, row_elem):
        ...
        colx = -1
        if self.verbosity >= 3:
            self.dumpout("<row> row_number=%r rowx=%d explicit=%d", row_number,
                         self.rowx, explicit_row_number)
        letter_value = _UPPERCASE_1_REL_INDEX
        for cell_elem in row_elem:
            ...
            if cell_name is None:  # Yes, it's optional.
                ...
                if self.verbosity and not self.warned_no_cell_name:
                    self.dumpout("no cellname; assuming rowx=%d colx=%d", rowx,
                                 colx)
                ...
                if explicit_row_number and cell_name[charx:] != row_number:
                    raise Exception('cell name %r but row number is %r' %
            ...
            if cell_type == 'n':
                ...
                if not tvalue:
                    ...
                    self.sheet.put_cell(rowx, colx, None, float(tvalue),
                                        xf_index)
            elif cell_type == "s":
                ...
                if not tvalue:
                    ...
                    self.sheet.put_cell(rowx, colx, XL_CELL_TEXT, value,
                                        xf_index)
            elif cell_type == "str":
                ...
                for child in cell_elem:
                    ...
                    if child_tag == V_TAG:
                        ...
                        bad_child_tag(child_tag)
                self.sheet.put_cell(rowx, colx, XL_CELL_BOOLEAN,
                                    cnv_xsd_boolean(tvalue), xf_index)
            elif cell_type == "e":
                # e = error
                ...
                if not tvalue:
                    ...
                    self.sheet.put_cell(rowx, colx, XL_CELL_TEXT, tvalue,
                                        xf_index)
            else:
